...
description: 在行为等价的前提下重构代码以降低理解成本。用户贴出代码要求简化、重构、去重、降低嵌套、改善命名或可读性时使用；追求更快更省的性能优化不在此列。
---
...
用户只说"优化一下"时先确认意图：要更快、更省内存、降低延迟是性能优化，交给 go-performance 或对应 senior-* skill；要更好读、更好改才用本 skill。与 go-review 等审查 skill 的分工：审查负责找 bug 与风险，本 skill 只在行为等价的前提下改表达方式。
