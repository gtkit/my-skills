...
description: Go 架构判断与方案取舍：必问的容量数字、单体与微服务判定、分层与包设计、泛型边界、存储/缓存/MQ 选型、分布式事务替代、稳定性预算、副本数估算。讨论 Go 项目设计、选型或评估方案时使用。
---
...
代码审查按 `go-review` 执行；自己刚写完的代码按 `go-enterprise-quality` 的四条自查（枚举输入域、新失败模式回审调用方、契约声明反证、改过的函数整段重读），并在交付说明里逐条体现。测试写法见 `go-testing`。
